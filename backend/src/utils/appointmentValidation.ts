...
import type { BookingPolicyValue, DoctorValue } from "../types";
...
const validateAppointmentSlot = (
  ...
  doctor: Pick<DoctorValue, "unavailableDates">,
): void => {
...
const assertNoDoctorConflict = async (
  doctorId: EntityId,
  appointmentDate: Date,
  excludeAppointmentId: EntityId | null = null,
): Promise<void> => {
...
const assertNoPatientConflict = async (
  ...
  const sameDoctorQuery: Record<string, unknown> = {
    doctorId,
    patientId,
    dateAndTime: {
  ...
  const sameTimeQuery: Record<string, unknown> = {
    patientId,
    dateAndTime: appointmentDate,
    status: { $ne: "cancelled" },
  };
