...
const UserSchema = new mongoose.Schema<UserDocumentShape>(
  {
    name: {
      type: String,
...
UserSchema.pre("save", async function (this: HydratedDocument<UserDocumentShape>) {
...
const filterDeleted = function (this: Query<unknown, UserDocumentShape>) {
...
export = mongoose.model<UserDocumentShape>("User", UserSchema);
