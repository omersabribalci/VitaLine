...
import type { RefreshTokenDocumentShape } from "../types";

const RefreshTokenSchema = new mongoose.Schema<RefreshTokenDocumentShape>({
...
//MongoDB TTL (Time-To-Live) indeksidir. expiresAt tarihi geldiği anda MongoDB bu belgeyi arka planda otomatik olarak veritabanından siler. TTL indeksi hem indeksi oluşturur hem de 0 saniye kala kaydı otomatik siler
RefreshTokenSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });

export = mongoose.model<RefreshTokenDocumentShape>("RefreshToken", RefreshTokenSchema);
