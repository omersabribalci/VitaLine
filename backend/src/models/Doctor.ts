...
import DateRangeSchema = require("./DateRange");
import type { DoctorDocumentShape } from "../types";

const DoctorSchema = new mongoose.Schema<DoctorDocumentShape>(
  {
    userId: {
...
const filterDeleted = function (this: Query<unknown, DoctorDocumentShape>) {
...
DoctorSchema.pre("aggregate", function (this: Aggregate<unknown[]>) {
  this.pipeline().unshift({ $match: { isDeleted: false } });
});

export = mongoose.model<DoctorDocumentShape>("Doctor", DoctorSchema);
