...
import type { AppRequest, UserDocumentShape } from "../types";
import type { LoginBody, RegisterPatientBody } from "../types/requests";

const registerPatient = async (
  req: AppRequest<RegisterPatientBody>,
  res: Response,
  next: NextFunction,
) => {
  ...
  try {
    ...
    await currentSession.withTransaction(async () => {
      ...
      patient = await Patient.create([{ userId: createdUser._id }], {
        session: currentSession,
      });
...
const refresh = async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const doc = await RefreshToken.findOne({
      tokenHash,
      jti: decoded.jti,
    }).populate<{ user: HydratedDocument<UserDocumentShape> }>("user");
    ...
    if (doc.revokedAt) {
      ...
      if (elapsed <= REFRESH_GRACE_MS && doc.replacedBy) {
        ...
        if (currentDoc && !currentDoc.revokedAt) {
          ...
          const { password: _password, ...plainUser } =
            currentDoc.user.toObject();
    ...
    const { password: _password, ...plainUser } = doc.user.toObject();
...
const logout = async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    const token = req.cookies?.refresh_token;
...
export { registerPatient, login, refresh, logout };
